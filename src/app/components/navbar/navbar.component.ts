...
export class NavbarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this._httpClient
      ...
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((result: { data: CountriesAndStates[] }) => {
        this.cs = result.data;
      });
  ...
  changeUserData(
    ...
    if (isImage && value instanceof HTMLInputElement) {
      ...
      fileReader.addEventListener('loadend', () => {
        const userImageElement = document.getElementsByName(
          `${path}Preview`
  ...
  async submitProfile(event: any) {
    ...
    if (!img) {
      await fetch(`${window.location.origin}/assets/defaultAvatar.png`)
        ...
        .then(
          (res) =>
            (img = new File([res], 'defaultAvatar.png', { type: res.type }))
        );
    }
